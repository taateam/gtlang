...
bool is_method_lv0(Mixed &_tokens_line1_arr, long _func_name_pos) {
	return (get_name(
			find_previous_non_spaces_token(_tokens_line1_arr, _func_name_pos))
			== ">>");
}
Mixed get_two_sides_of_method_call(Mixed _tokens_line1_arr,
...
Mixed get_function_and_params_lv0(Mixed &_tokens_line1_arr) {
	Mixed _return;
...
Mixed is_function_def(tokens_line _tokens) {
	...
	if (_tokens[0]._1 != "function" || _last_token._0 != ":"
...
Mixed process_user_function_def(long &_line_num,
		tokens_line1_arr _tokens_line1_arr, Mixed _func_declare,
...
long register_function(Mixed _parsed) {
	log("register_function() start");
	__funcs.push(_parsed);
	return __funcs.index;
}
	...
	for (auto [_key_rtt, _defined_func_auto] : __b_funcs) {
		...
		if (_key_rtt == _function_name) {
			try {
				...
				GtlangException _e1 = GtlangException(
...
// count required params;
	...
	if (gt_sizeof(_parsed["args"]) < _function["params"].valueNum) {
		throw GtlangException("FunctionParamsCountException",
				"function call shouldn't have more params than function define",
				_parsed);
	}
	...
	if (((unsigned long) gt_sizeof(_parsed["args"])) < _count_required_params)
		throw GtlangException("FunctionParamsCountException",
				"function call don't have enough params", _parsed);
...
// _parsed.del("args");
	...
	throw GtlangException("FunctionNotFoundException",
