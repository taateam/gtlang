...
Mixed convert_expr_to_complex_value(Mixed _object_tmp) {
	//return _object_tmp;
...
//	long _i;
	...
	try {
		...
		if (_start_vars_section == -1)
			throw GtlangException("SyntaxErrorException", "non-var in property",
					_tokens_line1_arr[0]);
...
Mixed is_method_def(tokens_line _tokens_line1_arr) {
	...
	if (_start_func_section == -1)
		throw GtlangException("SyntaxErrorException", "non-function in method",
				_tokens_line1_arr[0]);
...
Mixed parse_borrow(tokens_line _tokens_line1_arr_tl) {
	...
	for (auto [_k, _v] : _cutted.valueArray) {
		...
		if (get_type(_token) != "function")
			throw GtlangException("SyntaxErrorException", "Invalid borrow type",
					_token);
		Mixed _tmp;
...
Mixed parse_class(tokens_line1_arr _class_def_block) {
	...
	for (_i = 0; _i < gt_sizeof(_class_def_block); _i++) {
		...
		throw GtlangException("SyntaxErrorException", "wrong class internal",
				_line._0[0]);
	}
...
bool class_exists(string _class_name) {
	return __classes.contains(_class_name);
}
Mixed register_class(Mixed _new_class) {
	...
	if (class_exists(_class_name)) {
		return Mixed();
		throw GtlangException("ClassExistsException",
...
//	}
	if (isset(_new_class["extends"], 0)) {
		...
		if (!isset(__classes, _extend))
			throw GtlangException("ClassNotFoundException",
					"invalid extend class: " + _extend, _new_class);
		Mixed _tmp_extend, _extends;
		Mixed _parent_class = __classes[_extend];
	...
	for (auto [_k, _v] : _new_class["borrows"].valueArray) {
		...
		if (!isset(__classes, _borrowed_method_origin))
			throw GtlangException("ClassNotFoundException",
					"Cannot find original class to borrow", _borrow);
		if (!isset(__classes[_borrowed_method_origin]["methods"],
				_borrowed_method_name))
			throw GtlangException("MethodNotFoundException",
					"Cannot find method in original class to borrow", _borrow);
		Mixed _borrowed_method =
				__classes[_borrowed_method_origin]["methods"][_borrowed_method_name];
	...
	__classes.push(_class_name, _new_class);
	return Mixed();
}
Mixed call_user_method(string _class_name, Mixed &_properties, Mixed _parsed) {
...
// count required params;
	for (auto [_key_rtt, _defined_func_auto] : __classes[_class_name]["methods"].valueArray) {
	...
	if (gt_sizeof(_parsed["args"]) < _function["params"].valueNum) {
		throw GtlangException("FunctionParamsCountException",
				"method call can't have more params than function define",
				_parsed);
	}
	...
	if (((unsigned long) gt_sizeof(_parsed["args"])) < _count_required_params)
		throw GtlangException("function call don't have enough params",
				_parsed);
...
//	__current_func_lv--;
	for (auto [_k, _v] : _properties.valueArray) {
		_properties.replace(_k, __local_vars[_k]);
	}
	_return.push("properties", _properties);
	_return.push("return", _method_return);
	return _return;
}
void assign_properties(Mixed *_object, Mixed _properties) {
...
Mixed call_method(Mixed _parsed) {
	...
	Mixed _params;
	Mixed _return;
	Mixed _local_vars_backup = __local_vars;
...
//_parsed.del("args");
	...
	try {
		...
		__local_vars = _local_vars_backup;
		__current_func_lv--;
...
Mixed gt_new(Mixed _input) {
	...
	if (!isset(__classes, _class_name))
		throw GtlangException("ClassNotFoundException",
	...
	Mixed _class = __classes[_class_name];
