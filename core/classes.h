...
//	long _i;
	...
	try {
		...
		if (_start_vars_section == -1)
			throw GreenTeaException("SyntaxErrorException", "non-var in property",
					_tokens_line1_arr[0]);
...
Mixed is_method_def(tokens_line _tokens_line1_arr) {
	...
	if (_start_func_section == -1)
		throw GreenTeaException("SyntaxErrorException", "non-function in method",
				_tokens_line1_arr[0]);
...
Mixed parse_borrow(tokens_line _tokens_line1_arr_tl) {
	...
	for (auto [_k, _v] : _cutted.valueArray) {
		...
		if (get_type(_token) != "function")
			throw GreenTeaException("SyntaxErrorException", "Invalid borrow type",
					_token);
		Mixed _tmp;
...
//_parsed.del("args");
	...
	long _current_func_lv_bk = __current_func_lv;
	__current_func_lv++;
	try {
		...
		__current_func_lv = _current_func_lv_bk;
