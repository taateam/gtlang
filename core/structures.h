...
Mixed find_previous_non_spaces_token(Mixed &_tokens_line1_arr, long _pos) {
...
Mixed find_next_non_spaces_token(Mixed &_tokens_line1_arr, long _pos) {
	if (isset(_tokens_line1_arr, _pos + 1)) {
		Mixed _tmp = _tokens_line1_arr[_pos + 1];
		if (get_name(_tmp) != "spaces")
			return _tmp;
...
Mixed find_next_next_non_spaces_token(Mixed &_tokens_line1_arr, long _pos) {
	long _first_pos;
	if (isset(_tokens_line1_arr, _pos + 1)) {
		Mixed _tmp = _tokens_line1_arr[_pos + 1];
		if (get_name(_tmp) != "spaces")
...
arr_ll find_all_method_calls_at_level_0(Mixed &_tokens_line1_arr) {
	arr_ll _return;
...
Mixed array_cut_by_pos(Mixed _arr, arr_ll _positions) {
	Mixed _return;
//long _i_i = 0;
...
arr_ll find_all_token_type_at_level_0(string _token_type_to_compare,
		Mixed _tokens_line1_arr) {
	long _lv1;
	long _lv2;
	arr_ll _return;
...
arr_ll find_all_token_at_level_0(token _token_to_compare,
		Mixed _tokens_line1_arr) {
	long _lv1;
	long _lv2;
	arr_ll _return;
...
Mixed parse_array_element(Mixed _tokens_line1_arr) {
	Mixed _tmp_arr;
...
//return false;
//}
Mixed remove_all_spaces(Mixed _tokens_line1_arr) {
	Mixed _return;
	Mixed _ele;
//_return = ;
...
Mixed parse_var(Mixed _tokens_line1_arr) {
	...
	if (get_type(_tokens_line1_arr[0]) != "var"
		...
		throw GtlangException("SyntaxErrorException", "not a var",
				_tokens_line1_arr[0]);
...
// _tmp = Mixed();
	for (const auto& [_key_rtt, _token_auto] : _tokens_arr1.valueArray) {
		...
		if (_token == token("[", "operator")) {
			...
			throw GtlangException("SyntaxErrorException", "not valid var",
					_token);
	}
	return parse_expression(_tokens_line1_arr);
}
//Mixed parse_complex_var(Mixed _tokens_line1_arr) {
...
//	return parse_expression(_tokens_line1_arr);
//}
bool is_a_value_or_a_function(Mixed _complex_token) {
	string _type = get_type(_complex_token);
	return in_array(_type,
			{ { 0, "string" }, { 1, "var" }, { 2, "number" }, { 3, "array" }, {
					4, "object" }, { 5, "expr" }, { 6, "global_var" }, { 7,
					"object" }, { 8, "function" }, { 9, "method" }, { 10,
					"array" } });
}
bool is_a_class(Mixed _complex_token) {
	string _type = get_type(_complex_token);
	return in_array(_type, { { 0, "class" } });
}
bool is_a_var(Mixed _complex_token) {
	string _type = get_type(_complex_token);
	return in_array(_type, { { 0, "var" }, { 1, "global_var" } });
}
bool is_an_array(Mixed _complex_token) {
	string _type = get_type(_complex_token);
	return in_array(_type, { { 0, "array" } });
}
bool is_a_class_or_a_var(Mixed _complex_token) {
...
//		return false;
//}
bool _is_array_square_brackets(Mixed &_tokens_line1_arr,
...
Mixed parse_complex_var(Mixed _tokens_line1_arr) {
	...
	for (long _k = 0; _k < gt_sizeof(_tokens_line1_arr); _k++) {
		...
		if (_token == token(")", "operator")) {
			throw GtlangException("SyntaxErrorException", "Invalid ')'",
					_token);
		}
		if (_token == token("]", "operator")) {
			throw GtlangException("SyntaxErrorException", "Invalid ']'",
	...
	if (get_type(_expr_left_body[0]) != "var") {
		...
		if (!odd(gt_sizeof(_expr_left_body))) {
//			print_r(_expr_left_body);
			throw GtlangException("SyntaxErrorException",
					"Invalid array substitution.", _expr_left_body[0]);
		}
...
Mixed process_multiple_operators(Mixed _tokens_line1_arr_input) {
...
//	_tmp["body"] = _return;
//	return _tmp;
	throw InternalException("process_multiple_operators() shouldn't be here");
	return Mixed();
}
void validate_single_operator(Mixed _tokens_line1_arr_input) {
	...
	if (!__real_operators.contains(_operator_str))
		throw GtlangException("SyntaxErrorException", "Invalid operator",
				_operator_token);
...
Mixed is_list(Mixed _tokens, arr_ls _types,
		Mixed _delimiter = token(",", "operator")) {
	Mixed _return;
	bool _is_delimiter;
	token _token;
	Mixed _return1;
...
Mixed is_list(Mixed _tokens, string _type,
		Mixed _delimiter = token(",", "operator")) {
	return is_list(_tokens, { { 0, _type } }, _delimiter);
}
Mixed get_each_element_from_array(Mixed _arr, long _num = 1) {
	Mixed _return;
//	long _k;
	Mixed _token;
