...
template<typename V>
V get_last_ele(map<long, V> _arr) {
	if (!_arr.empty())
...
Mixed explode_tokens(token _del, tokens_line _tokens_line1_arr) {
	Mixed _return;
	long _k;
	_k = 0;
	Mixed _tmp;
...
Mixed explode_tokens(string _del, tokens_line _tokens_line1_arr) {
	Mixed _return;
	long _k;
	_k = 0;
	Mixed _tmp;
...
Mixed remove_spaces_from_tokens_arr(Mixed _tokens_line1_arr) {
	...
	for (auto [_key_rtt, _token_auto] : _tokens_line1_arr.valueArray) {
		...
		if ((get_name(_token) != "") && (get_type(_token) != "spaces"))
			_return.push(_token);
	}
	return _return;
}
