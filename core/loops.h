...
//	long __time;
	Mixed _return;
//	echo("");
	if (isset(_for_tokens, 2) && _for_tokens[2]._1 == "number"
			&& isset(_for_tokens, 4) && _for_tokens[4]._1 == "var"
			&& array_search(token(",", "operator"), _for_tokens) == -1) {
		_return.push(tokenize(_for_tokens[4]._0 + ":0")[0]._0);
		_return.push(
		...
		_return.push(tokenize("$_time<" + _for_tokens[2]._0)[0]._0);
	...
	if (gt_sizeof(_return) != 3)
		throw GtlangException("SyntaxErrorException", "invalid for line",
				get_1st_ele(_for_tokens));
	return _return;
}
Mixed process_foreach(long &_line_num, tokens_line1_arr _tokens_line1_arr) {
	Mixed _return, _arr, _k, _v;
	Mixed _foreach_line = trim_tokens_line_once(
			_tokens_line1_arr[_line_num]._0);
	...
	if (gt_sizeof(_all_at_pos) == 1) {
		...
		if (gt_sizeof(_after) > 2 || gt_sizeof(_after) < 1) {
			GtlangException("SyntaxErrorException",
					"Invalid foreach 1st line after at",
					get_1st_ele(_foreach_line));
		}
		if (gt_sizeof(_after) == 1) {
			_v = _after[0];
			if (!is_a_var(_v))
				throw GtlangException("SyntaxErrorException", "Not a var", _v);
		} else if (gt_sizeof(_after) == 3) {
			_k = _after[0];
			if (!is_a_var(_k))
				throw GtlangException("SyntaxErrorException", "Not a var", _k);
			_v = _after[2];
			if (!is_a_var(_v))
				throw GtlangException("SyntaxErrorException", "Not a var", _v);
		} else
