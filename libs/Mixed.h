...
public:
	...
	Mixed* get_e(string _k) {
		...
		throw InternalException("array dont have index: " + _k);
	}
...
//	}
	...
	string generate_ident(long _tab_lv = 0) {
		string _return;
		for (long _i = 0; _i < _tab_lv; _i++)
			_return += "\t";
		return _return;
	}
	string create_str(long _tab_lv = 0) {
		string _return;
		if (this->type == "s") {
	...
	string create_pretty_str(long _tab_lv = 0) {
		string _return;
		if (this->type == "s") {
			_return += this->valueStr;
			return _return;
...
arr_ll get_line_and_column_number(long _starting_pos, string _content);
class GtlangException {
public:
	Mixed obj;
	GtlangException(Mixed &_e) {
		obj = _e;
	}
	GtlangException(const char *_e_str) {
...
//	}
	GtlangException(string _class_name, string _e_str) {
		obj = Mixed();
		obj.push("class", "^" + string(_class_name));
		Mixed _property = Mixed();
		_property.push("$msg", string(_e_str));
	...
	GtlangException(string _class_name, string _e_str, string _token,
	...
	GtlangException(string _class_name, string _e_str, token _token) {
		obj = Mixed();
		obj.push("class", "^" + string(_class_name));
		Mixed _property = Mixed();
		_property.push("$msg", string(_e_str));
	...
	GtlangException(string _class_name, string _e_str, Mixed _token) {
		obj = Mixed();
		obj.push("class", "^" + string(_class_name));
		Mixed _property = Mixed();
		_property.push("$msg", string(_e_str));
	...
	GtlangException(string _e_str) {
		obj = Mixed(_e_str);
	}
};
class GtlangInnerFunctionException: public GtlangException {
public:
	GtlangInnerFunctionException(string _class_name, string _e_str) :
			GtlangException(_class_name, _e_str) {
	}
	;
};
...
//	return _arr;
//}
Mixed array_slice(Mixed _arr, long _start_pos, long _len = -1) {
	Mixed _return;
	long _end_pos = 0;
	if (_len > -1)
