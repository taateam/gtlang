...
// ];
		...
		if (php_to_string(_content.at(_i)) == "\n") {
			for (_i1 = 1; _i1 < _len - _i; _i1++) {
				__ = php_to_string(_content.at(_i + _i1));
				if (php_to_string(_content.at(_i + _i1)) == " "
						|| php_to_string(_content.at(_i + _i1)) == "\t") {
					continue;
				} else if (php_to_string(_content.at(_i + _i1)) == "\n") {
					_i += _i1;
				} else {
					break;
				}
...
// throw  ( "wrong string format" );
			...
			if (strlen(_extracted_str) == 1)
				throw GtlangException("SyntaxErrorException",
						"wrong var format", _extracted_str, _i, _content);
			tokens_push(_output_arr[_line]._0, { _extracted_str, "var" }, _i,
					_content);
			_i += _i1 - 1;
			...
			tokens_push(_output_arr[_line]._0, { _extracted_str, "function" },
					_i, _content);
			_i += _i1 - 1;
		} else if (_content[_i] == '^') {
			for (_i1 = 1; _i1 < _len - _i; _i1++) {
			...
			tokens_push(_output_arr[_line]._0, { _extracted_str, "class" }, _i,
					_content);
			_i += _i1 - 1;
			...
			for (_i1 = 1; _i1 < _len - _i; _i1++) {
				if (php_to_string(_content.at(_i + _i1)) == "\\") {
					if (php_to_string(_content.at(_i + _i1 + 1)) == "n") {
						_extracted_str += "\n";
						_i1 += 1;
					} else if (php_to_string(_content.at(_i + _i1 + 1))
							== "t") {
						_extracted_str += "\t";
						_i1 += 1;
					} else if (php_to_string(_content.at(_i + _i1 + 1))
...
// throw  ( "wrong string format" );
			...
			_i += _i1;
			if (!_flag)
				throw GtlangException("SyntaxErrorException", "Unexpected EOF",
						_extracted_str, _i, _content);
		} else if (_content[_i] == '\'') {
			_extracted_str = "";
			bool _flag = false;
			for (_i1 = 1; _i1 < _len - _i; _i1++) {
				if (php_to_string(_content.at(_i + _i1)) == "\\") {
					if (php_to_string(_content.at(_i + _i1 + 1)) == "\\") {
						_extracted_str += "\\";
						_i1 += 1;
					} else if (php_to_string(_content.at(_i + _i1 + 1))
							== "'") {
			...
			if (in_array(_extracted_str, __keywords))
				tokens_push(_output_arr[_line]._0, { l(_extracted_str),
						"keyword" }, _i, _content);
			else if (in_array(_extracted_str, __modifiers))
				tokens_push(_output_arr[_line]._0, { l(_extracted_str),
						"modifier" }, _i, _content);
			else if (_extracted_str == "true")
				tokens_push(_output_arr[_line]._0, { "true", "bool" }, _i,
						_content);
			else if (_extracted_str == "false")
			...
			if (!check)
				tokens_push(_output_arr[_line]._0,
						{ php_to_string(_content.at(_i)), "other0" }, _i,
						_content);
		} else {
			tokens_push(_output_arr[_line]._0, { php_to_string(_content.at(_i)),
					"other" }, _i, _content);
		}
	}
...
bool _has_class(Mixed _obj, string _class_name) {
	string _class_of_obj = _obj["class"];
	if (_class_of_obj == _class_name)
		return true;
	Mixed _extends = __classes[_class_of_obj]["extends"];
	bool _return = false;
...
//	print_r(_parsed);
	...
	if (get_type(_parsed) == "number") {
		return Mixed(tl(get_name(_parsed)));
	}
...
//array's element;
	...
	throw GtlangException("SyntaxErrorException", "unsupported expr", _parsed);
}
...
//	print_r(_cmd);
	if (_cmd["type"] == "throw") {
		...
		throw GtlangException(_exception);
...
//				gt_exec(_cmd["finally"]["body"]);
		...
		if (gt_sizeof(_cmd["body"]) == 1) {
			...
			throw GtlangException("SyntaxErrorException", "incorrect break",
					_cmd["body"][0]);
	} else if (isset(_cmd, "body") && isset(_cmd["body"], 0)
		...
		if (gt_sizeof(_cmd["body"]) == 1) {
			...
			throw GtlangException("SyntaxErrorException", "incorrect continue",
					_cmd["body"][0]);
...
//	}
	...
	if (_opt == "[]") {
		...
		if ((double) _param1_finalized == 0)
			throw GtlangException("DivisionByZeroException",
		...
		return _param0_finalized.valueBool && _param1_finalized.valueBool;
	} else if (_opt == "||") {
		return _param0_finalized.valueBool || _param1_finalized.valueBool;
	} else if (_opt == "!") {
		return !_param0_finalized.valueBool;
	} else if (_opt == "==") {
		return _param0_finalized == _param1_finalized && _params[0][1];
	} else if (_opt == "=") {
		...
		assign_var(_params[0][0],
				_return = _param0_finalized + _param1_finalized);
		return _return;
	} else if (_opt == "-:") {
		assign_var(_params[0][0],
				_return = _param0_finalized - _param1_finalized);
		...
		throw GtlangException("SyntaxErrorException", "unknown operator",
				_params[1]);
	}
}
//=============================================================================================
